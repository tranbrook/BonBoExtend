    ...
    pub fn new() -> Self {
        Self {
            metadata: PluginMetadata {
                ...
                description: "Position sizing, circuit breakers, risk metrics".into(),
                author: "BonBo Team".into(),
                tags: vec!["risk".into(), "position_sizing".into()],
            },
        }
    }
}
...
impl ToolPlugin for RiskPlugin {
    ...
    fn tools(&self) -> Vec<ToolSchema> {
        vec![
            ToolSchema {
                name: "calculate_position_size".into(),
                description: "Calculate optimal position size".into(),
                parameters: vec![
                    ParameterSchema {
                        name: "equity".into(),
                        param_type: "number".into(),
                        description: "Portfolio equity USDT".into(),
                        required: true,
                        default: None,
                        r#enum: None,
                    },
                    ...
                    ParameterSchema {
                        ...
                        description: "Sizing method".into(),
                        required: false,
                        default: Some(json!("fixed_percent")),
                        r#enum: Some(vec![
                            "fixed_percent".into(),
                            "kelly".into(),
                            "half_kelly".into(),
                    ...
                    ParameterSchema {
                        name: "risk_pct".into(),
                        param_type: "number".into(),
                        description: "Risk % per trade".into(),
                        required: false,
                        default: Some(json!(0.02)),
                        r#enum: None,
                    },
                ],
            },
            ToolSchema {
                name: "compute_risk_metrics".into(),
                description: "Compute portfolio risk metrics: VaR, CVaR, Sharpe, Max Drawdown"
                    .into(),
                parameters: vec![
                    ParameterSchema {
                        name: "trade_pnls".into(),
                        param_type: "array".into(),
                        description: "Array of trade PnL values".into(),
                        required: true,
                        default: None,
                        r#enum: None,
                    },
                    ParameterSchema {
                        name: "equity_curve".into(),
                        param_type: "array".into(),
                        description: "Equity values over time".into(),
                        required: true,
                        default: None,
                        r#enum: None,
                    },
                    ParameterSchema {
                        ...
                        param_type: "number".into(),
                        description: "Initial capital".into(),
                        required: false,
            ...
            ToolSchema {
                ...
                description: "Check if trading is allowed via circuit breaker".into(),
                parameters: vec![
                    ParameterSchema {
                        name: "equity".into(),
                        param_type: "number".into(),
                        description: "Current equity".into(),
                        required: true,
                        default: None,
                        r#enum: None,
                    },
                    ParameterSchema {
                        name: "initial_capital".into(),
                        param_type: "number".into(),
                        description: "Starting capital".into(),
                        required: true,
                        default: None,
                        r#enum: None,
                    },
                    ParameterSchema {
                        name: "daily_pnl".into(),
                        param_type: "number".into(),
                        description: "Today's P&L".into(),
                        required: false,
                        default: Some(json!(0)),
                        r#enum: None,
                    },
                    ParameterSchema {
                        name: "peak_equity".into(),
                        param_type: "number".into(),
                        description: "Highest equity".into(),
                        required: false,
                        default: None,
                        r#enum: None,
                    },
                    ParameterSchema {
                        ...
                        description: "Consecutive losses".into(),
                        required: false,
                        default: Some(json!(0)),
                        r#enum: None,
                    },
...
impl RiskPlugin {
    fn calc_position(&self, args: &Value) -> anyhow::Result<String> {
        ...
        let entry = args["entry_price"]
            .as_f64()
            .ok_or_else(|| anyhow::anyhow!("entry_price required"))?;
        let stop = args["stop_loss"]
            .as_f64()
            .ok_or_else(|| anyhow::anyhow!("stop_loss required"))?;
        ...
        let sizing = match method {
            "kelly" => bonbo_risk::position_sizing::SizingMethod::Kelly {
                win_rate: 0.55,
                avg_win: 200.0,
                avg_loss: 100.0,
            },
            "half_kelly" => bonbo_risk::position_sizing::SizingMethod::HalfKelly {
                win_rate: 0.55,
                avg_win: 200.0,
                avg_loss: 100.0,
            },
    ...
    fn calc_metrics(&self, args: &Value) -> anyhow::Result<String> {
        ...
        if pnls.is_empty() || eq_curve.is_empty() {
            return Ok("⚠️ Empty data".to_string());
        }
        let m = bonbo_risk::var::compute_portfolio_metrics(&pnls, &eq_curve, initial);
        Ok(format!(
            "📊 **Risk Metrics**\n\n💰 Equity: ${:.2}\n📈 Return: {:.2}%\n\n**Risk:**\n  VaR(95%): {:.2}%\n  CVaR(95%): {:.2}%\n  Max DD: {:.2}%\n\n**Performance:**\n  Sharpe: {:.2}\n  Sortino: {:.2}\n  Win Rate: {:.1}%\n  Profit Factor: {:.2}\n  Avg Trade: ${:.2}",
            m.current_equity,
            m.total_return_pct * 100.0,
            m.var_95 * 100.0,
            m.cvar_95 * 100.0,
    ...
    fn check_risk(&self, args: &Value) -> anyhow::Result<String> {
        let equity = args["equity"]
            .as_f64()
            .ok_or_else(|| anyhow::anyhow!("equity required"))?;
        let initial = args["initial_capital"]
            .as_f64()
            .ok_or_else(|| anyhow::anyhow!("initial_capital required"))?;
