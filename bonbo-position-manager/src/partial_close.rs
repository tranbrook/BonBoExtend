...
impl PartialCloseManager {
    ...
    pub fn tp1_quantity(position: &ManagedPosition) -> Decimal {
        ...
        (position.original_quantity * pct / Decimal::ONE_HUNDRED).round_dp(4)
    }
    ...
    pub fn tp2_quantity(position: &ManagedPosition) -> Decimal {
        ...
        (position.original_quantity * Decimal::from(tp2_pct) / Decimal::ONE_HUNDRED).round_dp(4)
    }
    ...
    pub async fn execute_partial_close(
        ...
        let response =
            bonbo_binance_futures::rest::OrdersClient::place_order(rest_client, &order).await?;
