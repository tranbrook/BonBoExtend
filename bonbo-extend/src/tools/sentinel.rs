...
impl SentinelPlugin {
    pub fn new() -> Self {
        Self {
            metadata: PluginMetadata {
                id: "bonbo-sentinel".into(),
                name: "Sentinel — On-chain & Sentiment".into(),
                version: env!("CARGO_PKG_VERSION").into(),
                description: "On-chain analytics, sentiment analysis, whale alerts".into(),
                author: "BonBo Team".into(),
                tags: vec!["sentiment".into(), "on-chain".into()],
            },
        }
    }
}
...
impl ToolPlugin for SentinelPlugin {
    ...
    fn tools(&self) -> Vec<ToolSchema> {
        vec![
            ToolSchema {
                name: "get_fear_greed_index".into(),
                description: "Get crypto Fear & Greed Index".into(),
                parameters: vec![ParameterSchema {
                    name: "history".into(),
                    param_type: "integer".into(),
                    description: "Days of history (1-30)".into(),
                    required: false,
                    default: Some(json!(1)),
                    r#enum: None,
                }],
            ...
            ToolSchema {
                name: "get_whale_alerts".into(),
                description: "Get recent large crypto transactions (simulated)".into(),
                parameters: vec![ParameterSchema {
                    name: "min_usd".into(),
                    param_type: "number".into(),
                    description: "Min USD value".into(),
                    required: false,
            ...
            ToolSchema {
                name: "get_composite_sentiment".into(),
                description: "Composite market sentiment from multiple sources".into(),
                parameters: vec![ParameterSchema {
                    name: "symbol".into(),
                    param_type: "string".into(),
                    description: "Symbol for context".into(),
                    required: false,
                    default: Some(json!("BTCUSDT")),
                    r#enum: None,
                }],
            },
        ]
...
impl SentinelPlugin {
    ...
    fn get_whale_alerts(&self, args: &Value) -> anyhow::Result<String> {
        ...
        for tx in &txs {
            let dir = if tx.is_exchange_inflow {
                "🔴 Inflow (sell pressure)"
            ...
            result.push_str(&format!(
                ...
                tx.token,
                tx.blockchain,
                &tx.from_addr[..10.min(tx.from_addr.len())],
                &tx.to_addr[..10.min(tx.to_addr.len())]
            ));
        }
        Ok(result)
    }
    ...
    async fn get_composite(&self, args: &Value) -> anyhow::Result<String> {
        ...
        let whale_signal = whale_txs
            .as_ref()
            .and_then(|txs| bonbo_sentinel::WhaleAlertFetcher::to_sentiment_signal(txs));
        let mut signals: Vec<bonbo_sentinel::models::SentimentSignal> = Vec::new();
        if let Some(s) = &fg_signal {
            signals.push(s.clone());
        }
        if let Some(s) = &whale_signal {
            signals.push(s.clone());
        }
        ...
        result.push_str("\n💡 ");
        if report.composite_score > 0.3 {
            result.push_str("Bullish sentiment — consider longs with risk management.");
        } else if report.composite_score < -0.3 {
            result.push_str("Bearish sentiment — defensive positioning advised.");
        } else {
            result.push_str("Neutral sentiment — wait for clearer signals.");
        }
        Ok(result)
    }
}
