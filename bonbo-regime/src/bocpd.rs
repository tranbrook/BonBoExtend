...
impl BocpdDetector {
    ...
    pub fn update(&mut self, value: f64, timestamp: i64) -> Option<ChangePoint> {
        ...
        if self.baseline_n == 0 {
            ...
            self.baseline_var = (1.0 - alpha_baseline) * self.baseline_var + alpha_baseline * delta * delta;
        }
        self.baseline_n += 1;
        ...
        let change_detected = if self.recent_values.len() >= 10 && self.baseline_n >= 20 {
            let recent_mean: f64 = self.recent_values.iter().sum::<f64>() / self.recent_values.len() as f64;
            let baseline_std = self.baseline_var.sqrt().max(1e-6);

            // Z-score: how far is recent mean from baseline
            let z_score = (recent_mean - self.baseline_mean).abs() / baseline_std * (self.recent_values.len() as f64).sqrt();

            // Also check if recent variance is much higher
            let recent_var: f64 = self.recent_values.iter().map(|v| (v - recent_mean).powi(2)).sum::<f64>() / self.recent_values.len() as f64;
            let var_ratio = if self.baseline_var > 1e-10 { recent_var / self.baseline_var } else { 1.0 };
        ...
        for stats in &self.stats {
            ...
            let pred = (-exponent.exp() / (sigma * (2.0 * std::f64::consts::PI).sqrt())).max(1e-300);
            preds.push(pred);
        }
        ...
        let cp_prob: f64 = self.run_length_probs.iter()
            .zip(preds.iter())
            .map(|(&p, &pred)| p * pred * h)
            .sum();
        new_probs[0] = cp_prob;
        ...
        if new_probs.len() > self.max_run_length {
            ...
            if sum > 0.0 { for p in &mut new_probs { *p /= sum; } }
        }
        ...
        if change_detected || bocpd_cp_probability > 0.5 {
            ...
            let cp = ChangePoint {
                timestamp,
                index: self.index,
                confidence: if change_detected { 0.8 } else { bocpd_cp_probability },
    ...
    fn classify_current_regime(&self) -> MarketRegime {
        ...
        let best_idx = self.run_length_probs.iter()
            .enumerate()
    ...
    pub fn get_state(&self, timestamp: i64) -> RegimeState {
        ...
        let volatility = self.stats.first().map(|s| s.variance().sqrt()).unwrap_or(0.01);
        let trend = self.stats.first().map(|s| s.mean()).unwrap_or(0.0);

        let mut probs = HashMap::new();
        probs.insert(MarketRegime::Volatile, if volatility > self.config.volatile_threshold { 0.4 } else { 0.05 });
        probs.insert(MarketRegime::Quiet, if volatility < self.config.quiet_threshold { 0.4 } else { 0.05 });
        probs.insert(MarketRegime::TrendingUp, if trend > self.config.trend_threshold { 0.5 } else { 0.1 });
        probs.insert(MarketRegime::TrendingDown, if trend < -self.config.trend_threshold { 0.5 } else { 0.1 });
        probs.insert(MarketRegime::Ranging, 0.3);

        // Normalize
        let sum: f64 = probs.values().sum();
        let regime_probabilities: Vec<(MarketRegime, f64)> = probs.into_iter()
            .map(|(r, p)| (r, p / sum))
            .collect();

        let confidence = regime_probabilities.iter()
...
mod tests {
    ...
    fn test_bocpd_regime_change() {
        ...
        assert!(det.change_points().is_empty() || det.change_points().len() <= 2,
        ...
        assert!(detected_count >= 1, "BOCPD should detect at least one regime change after drastic shift, got {}", detected_count);
    }
