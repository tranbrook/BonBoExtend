...
impl Default for LearningPlugin {
    fn default() -> Self {
        Self::new()
    }
}
...
impl ToolPlugin for LearningPlugin {
    ...
    fn tools(&self) -> Vec<ToolSchema> {
        vec![
            ToolSchema {
                name: "get_scoring_weights".into(),
                description: "Current learned scoring weights (adapted from past outcomes)".into(),
                parameters: vec![],
            },
            ToolSchema {
                name: "get_learning_stats".into(),
                description: "DMA learning stats — shows what the engine has learned".into(),
                parameters: vec![],
            },
            ToolSchema {
                name: "reset_learning".into(),
                description: "Reset DMA learning back to defaults".into(),
                parameters: vec![],
            },
        ]
    }
        ...
        match tool_name {
            "get_scoring_weights" => {
                ...
                result.push_str(&format!(
                    "🧠 **Learned (adapted from past trades):**\n{}\n",
                    fw(&learned)
                ));
                ...
                for regime in &["TrendingUp", "Ranging", "Volatile"] {
                    result.push_str(&format!(
                        "\n🎯 **{} Regime:**\n{}\n",
                        regime,
                        fw(&ScoringWeights::for_regime(regime))
                    ));
                }
            ...
            "get_learning_stats" => {
                ...
                result.push_str(&format!(
                    ...
                    if dma.should_revert_to_defaults() {
                        "⚠️ YES — accuracy too low"
                    } else {
                        "✅ No"
                    }
                ));
