//! BonBoExtend — DOT Full Quantitative Analysis
//!
//! Uses all BonBoExtend crates for deep quantitative analysis:
//! - bonbo-data:   Fetch real-time DOT klines from Binance API
...
//!
//! Usage: cargo run -p bonbo-extend --example dotusdt_analysis
...
async fn main() -> Result<()> {
    separator("BONBO EXTEND — DOTUSDT (POLKADOT) FULL QUANTITATIVE ANALYSIS");
    println!("  Symbol: DOTUSDT");
    println!(
        "  Generated: {}",
    ...
    if let Some(latest) = raw_candles.last() {
        ...
        println!("  📊 Volume:    {:.0} DOT", latest.volume);
        println!(
            "  🕐 Time:      {}",
    ...
    // ══════════════════════════════════════════════════
    separator("FINAL SUMMARY — 30-DAY DOT OUTLOOK");
